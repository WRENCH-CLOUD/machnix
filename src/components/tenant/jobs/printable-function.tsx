...
export const usePrintableFunctions = ({
  ...
  const invoicePrintData = useMemo(
    () =>
      buildInvoicePrintData({
        job,
        estimateItems,
    ...
    [job, estimateItems, invoice, tenantDetails, isGstBilled, discountPercentage]
  );

  const resolvedInvoiceTemplate = useMemo(
    () => resolveInvoiceTemplate("auto", invoicePrintData),
    [invoicePrintData]
  );
  ...
  const handleGenerateInvoicePdf = () => {
    ...
    const source = document.querySelector(".invoice-print-root") as HTMLElement | null;
    if (!source) {
      window.alert("Unable to print invoice preview. Please reopen the invoice tab and try again.");
      return;
    }

    const staging = document.createElement("div");
    staging.className = "invoice-print-staging";
